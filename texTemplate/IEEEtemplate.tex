...
Simon Cipher ~\cite{Beaulieu_Simon} is a Feistel cipher consisting in the following operations: bitwise XOR, $\oplus$ ; bitwise AND, $\And$ ; left circular shift S by j bits, $S^j$ . Because Simon has been designed to be a good candidate for different platforms that may have more or less resources and may need to be very secure or not that secure, multiple versions that operate on different block sizes and key sizes of the cipher have been introduced. The first implementation used in this study is Simon64/128. It consists in a block size of 64 bits, a word size of 32 bits, and a key size of 128 bits. In this implementation, the Feistel operations are being performed in 44 rounds .
